...
export const listHandle = (event: MouseEvent<HTMLButtonElement>, jwt: string,
    ...
    fetch("http://" + location.host + "/api/users/fr-bl-list", {
        ...
        body: JSON.stringify({
            userId: Number(userInfo.id), type: 2
        })
    }).then(res => {
        if (res.ok)
            return (res.json());
...
const ButtonsInfos = (props: typeButtonsInfo) => {
    ...
    return (<>
        <button onClick={(e: React.MouseEvent<HTMLButtonElement>) =>
            ...
            className="userInfo">{(props.userInfo.fl === 2 ? "Remove" : "Add")} friend</button>
		<button className="userInfo">Invite to a game</button>
		<button className="userInfo">User Profile</button>
		<button className="userInfo">Direct message</button>
    </>)
}

const Display = (props: {jwt: string, lstUserGlobal: Array<typeFlBl>,
	userInfo: typeUserInfo,
	setUserInfo: React.Dispatch<React.SetStateAction<typeUserInfo>>,
	setErrorCode: React.Dispatch<React.SetStateAction<number>>,}) => {
	...
	return (<>
		...
		<div className={chooseClassName} style={{position: "relative"}}>
			...
			<ButtonsInfos jwt={props.jwt} userInfo={props.userInfo}
